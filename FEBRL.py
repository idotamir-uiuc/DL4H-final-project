...
for comb in itertools.combinations(range(len(model_raw_score)), 3):
    selected_raw_scores = np.copy(model_raw_score[list(comb)])
    selected_models = model_names[list(comb)]
...
#     for thres2 in np.arange(0.1, 1, 0.1):
#         for comb in itertools.combinations(range(len(model_raw_score)), 3):
#             selected_raw_scores = np.copy(model_raw_score[list(comb)])
#             selected_models = model_names[list(comb)]
#             selected_raw_scores[selected_raw_scores > thres2] = 1
#             selected_raw_scores[selected_raw_scores <= thres2] = 0
#             stack_raw_score = np.average(selected_raw_scores, axis=0)
#             stack_binary_score = np.copy(stack_raw_score)
...
#             stacking_eval = evaluate_results(y_test, stack_binary_score)
# #             print(*selected_models, sep=", ")
#             if max_f_score < stacking_eval.f_score:
#                 max_f_score = stacking_eval.f_score
#                 max_thres = thres
#                 max_thres2 = thres2
#
# print(max_thres, max_thres2, max_f_score)